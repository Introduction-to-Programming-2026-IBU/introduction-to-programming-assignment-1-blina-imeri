...
SELECT first_name, last_name FROM student WHERE enrollment_year = 2021;

-- 1.5: Find students with GPA between 3.0 and 3.5 (inclusive)

SELECT first_name, last_name, gpa FROM students WHERE gpa >= 3.0 AND gpa <= 3.5;

-- 1.6: Find the student with email 'grace@school.edu'

SELECT first_name, last_name FROM students WHERE email = 'grace@school.edu';

-- 1.7: Retrieve only the first 5 students (by id)

SELECT first_name, last_name FROM students ORDER BY id ASC LIMIT 5;
